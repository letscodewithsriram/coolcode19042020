...
do
	...
	CPU=`/usr/bin/snmpwalk -v 2c -c public 192.168.0.110 1.3.6.1.4.1.9.2.1.58 | cut -f4 -d' '`
	#!# `curl -i -XPOST http://localhost:8086/write?db=na --data-binary "lab,device=$HOSTNAME,kpi=cpu value=$CPU $DBS_TIME"`

	# Memory

	MEM=`/usr/bin/snmpwalk -v 2c -c public 192.168.0.110 1.3.6.1.4.1.9.9.48.1.1.1.5.1 | cut -f4 -d' '`
	#!# `curl -i -XPOST http://localhost:8086/write?db=na --data-binary "lab,device=$HOSTNAME,kpi=mem value=$MEM $DBS_TIME"`

	# Temperature

	TEMP=`/usr/bin/snmpwalk -v 2c -c public 192.168.0.110 1.3.6.1.4.1.9.9.13.1.3.1.6 | cut -f4 -d' '`
	#!# `curl -i -XPOST http://localhost:8086/write?db=na --data-binary "lab,device=$HOSTNAME,kpi=temp value=$TEMP $DBS_TIME"`

	# Power
	
	POW=`/usr/bin/snmpwalk -v 2c -c public 192.168.0.110 1.3.6.1.4.1.9.9.13.1.5.1.3.1 | cut -f4 -d' '`
...
influx -import -path=datapts.txt -precision=ms -database=na
